...
# LDA analysis (here we get the 5 more common topics in the texts)
...
# Save topics and common words to a file
...
plt.bar(*zip(*word_freq_dist.most_common(20)))
...
# Plot filler word counts
plt.figure(figsize=(10, 5))
filler_word_counts_values = list(filler_word_counts.values())
plt.bar(filler_word_counts.keys(), filler_word_counts_values, color='lightblue')
plt.title('Filler Word Counts')
plt.xlabel('Filler Words')
plt.ylabel('Frequency')
plt.xticks(rotation=45)
for i, (word, count) in enumerate(filler_word_counts.items()):
